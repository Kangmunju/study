...
# 1. 회원 카드 만들기
card_name = input("이름: ")
card_age = int(input("나이: "))

# 미성년자확인
adult_check = card_age >= 19
card = {"name": card_name, "age": card_age, "adult": adult_check}

print(card)
print(f"{card_name}님 / {card_age}세 / 성인 여부: {adult_check}")
...
drink = input("아메리카노/라떼/케이크 : ")
cup_count = int(input("잔 수 : "))
fee = int(menu[drink]["price"]) * cup_count
total_kc = int(menu[drink]["kcal"]) * cup_count

print(f"{drink} X {cup_count} = {fee} / {total_kc}칼로리")
print("무료 배송 대상입니다.")


# 4. 숫자 분석 딕셔너리
num_in = int(input("숫자 입력 : "))
if num_in % 2 == 0:
    even_check = True
else:
    even_check = False
...
print(num_dic)
if even_check:
    print(f"{num_in}은(는) 짝수입니다.")
else:
    print(f"{num_in}은(는) 홀수입니다.")
...
b_dic = {
    "오만원권": money // 50000,
    "만원권": (money % 50000) // 10000,
    "천원권": (money % 10000) // 1000,
}

print(f"오만원권 {b_dic['오만원권']}장")
print(f"만원권 {b_dic['만원권']}장")
print(f"천원권 {b_dic['천원권']}장")

# 남은 돈 계산
remain_m = (
    money
    - (50000 * list(b_dic.values())[0])
    - (10000 * list(b_dic.values())[1])
    - (1000 * list(b_dic.values())[2])
)
print(f"남은 돈 {remain_m}원")


# 6. BMI 계산기
weight = float(input("몸무게 : "))
hegith = float(input("키 : "))
bmi = weight / (hegith * hegith)

bmi_dic = {"키": hegith, "몸무게": weight}
if bmi >= 25:
    bmi_dic["판정"] = "비만"
elif 23 <= bmi < 25:
    bmi_dic["판정"] = "과체중"
...
final_b_dic = {"bmi": bmi, "판정": bmi_dic["판정"]}

print(final_b_dic)
print(f"BMI {bmi:.2f} -> {bmi_dic['판정']}")


# 7. 초를 시분초로 바꾸기
sec = int(input("초 단위 정수 : "))
hour = sec // 3600
minute = (sec - 3600) // 60
remain_sec = sec - (3600 * hour) - (60 * minute)
print(f"{sec}초 = {hour}시간 {minute}분 {remain_m}초")
...
# 입력받은 음료가 자판기에 없으면 거름
# 제품은 있는데 재고 0이면 거름
# 제품도 있고 재고도 있는데 금액이 모자람 -> 얼마가 부족한지 출력
# 전부 만족 -> 재고 -1 -> 예시대로 출력

if not drink_name in vending:
    print("다른 음료 선택")
else:
    if vending[drink_name]["stock"] == 0:
        print("재고가 0인 상품")
    elif vending[drink_name]["stock"] != 0 and vending["price"] > loan:
        print(f"금액 부족 : {vending['price'] - loan}")
    else:
        print(f"{drink_name} 구매 완료 / 거스름돈 {loan - vending['price']}")
        print(f"{drink_name} 남은 재고 : {vending['stock'] - 1}")
