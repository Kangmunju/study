...
# [이 파일의 용도]
# 지금까지 만든 유용한 함수들을 한 곳에 모아 둔 파일
# '실행하는 파일'이 아닌 '가져다 쓰는 파일'
# import 와 s1.py에서 사용하게 될 것
# ex. import my_tools
#     my_tools.to_int("100")

# [파일을 따로 나누는 이유]
# 같은 함수를 프로젝트마다 복사해서 사용하게 되면
# 나중에 고칠 때 모든 파일을 전부 찾아서 고쳐야 함
# 한 파일에 모아두게 되면
# - 여러 프로젝트에서 재사용 가능
# - 수정할 부분이 적어짐
# - 다른 사람에게 파일만 전달해주면 됨

# pandas, numpy 또한 같은 원리로 만들어져있는 것
# 누군가 유용한 함수를 모아서 배포해둔 것
...
# 단위와 쉼표를 제거하고 숫자만 뽑아내기
# 사용 예 ) clean_number("4,500원") -> 4500
#          clean_number(" 30개 ") -> 30
#          clean_number("없음") -> None
# 실무데이터에서는 '4,500원'처럼 생긴 데이터 값이 흔함
# to_int로는 실패하지만 이 함수를 사용하면 해결
